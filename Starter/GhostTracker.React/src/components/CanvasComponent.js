...
function CanvasComponent() {
  ...
  useEffect(() => {
    ...
    ghost2.src = './public/ghost2.png';
    ghost2.onload = () => {
      console.log('Ghost 2 loaded');
    };
  ...
  const renderCanvas = async () => {
    if (!imageMemory 
      ...
      || !imageMemory.ghost1.complete
      || !imageMemory.ghost2.complete) return;
    ...
    ghostSummary.ghosts.forEach(ghost => {
      ...
      if (ghost.id % 2 == 0) {
        ctx.drawImage(imageMemory.ghost2, ghost.x - GHOST_SIZE_HALF, ghost.y - GHOST_SIZE_HALF, GHOST_SIZE, GHOST_SIZE);
      }
      else {
